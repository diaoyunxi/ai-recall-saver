...
> 本扩展未上架 Chrome 应用商店，采用「加载已解压的扩展程序」方式安装。

1. 从 [Releases](https://github.com/diaoyunxi/ai-recall-saver/releases) 下载最新的 `ai-recall-saver-vX.X.X.zip` 并**解压**到任意目录。
   （或 `git clone` 本仓库，效果相同）
2. 打开 Chrome，地址栏输入 `chrome://extensions/`。
3. 打开右上角「开发者模式」。
4. 点击「加载已解压的扩展程序」，选择解压后的**根目录**（含 `manifest.json` 的目录）。
5. 访问任一支持的 AI 网页版即可自动生效。

```bash
git clone https://gh.llkk.cc/https://github.com/diaoyunxi/ai-recall-saver.git
```

> 💡 更新版本时：重新下载解压新 ZIP，在 `chrome://extensions/` 点击扩展卡片的「刷新」按钮即可。
...
本项目提供 `scripts/pack_zip.py` 可一键生成可直接加载的 ZIP 包（不含开发脚本与私钥）：

```bash
python scripts/pack_zip.py
```

输出文件位于 `dist/ai-recall-saver-v<版本>.zip`，解压后即可用「加载已解压的扩展程序」安装。
...
**Q：安装时提示「非官方扩展」或「开发者模式」怎么办？**
A：本扩展未上架 Chrome 商店，通过「加载已解压的扩展程序」安装。Chrome 顶部会持续提示「停用开发者模式扩展」，这是正常现象，不影响使用。如需消除提示，可将扩展固定到工具栏。
